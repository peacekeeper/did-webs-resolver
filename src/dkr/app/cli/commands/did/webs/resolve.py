...
class WebsResolver(doing.DoDoer):
    ...
    def resolve(self, tymth, tock=0.0, **opts):
        ...
        replaced_path = path.replace(':', '/') if path is not None else None
        base_url = f"http://{domain}{(f':{port}' if port is not None else '')}{(f'/{replaced_path}' if replaced_path is not None else '')}/{aid}"
